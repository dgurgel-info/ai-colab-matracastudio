# Higgs Audio V2 Tokenizer compatibility bridge for Transformers 4.x
# Enables OmniVoice (k2-fsa) to run alongside Qwen3-TTS on Transformers 4.57.3
...
from transformers import PretrainedConfig as PreTrainedConfig, PreTrainedModel as PreTrainedAudioTokenizerBase, AutoModel
from transformers.utils import ModelOutput
from transformers.models.auto import CONFIG_MAPPING, AutoConfig

def conv1d_output_length(module, input_length):
    p = module.padding[0] if isinstance(module.padding, (tuple, list)) else module.padding
    d = module.dilation[0] if isinstance(module.dilation, (tuple, list)) else module.dilation
    k = module.kernel_size[0] if isinstance(module.kernel_size, (tuple, list)) else module.kernel_size
    s = module.stride[0] if isinstance(module.stride, (tuple, list)) else module.stride
    return (input_length + 2 * p - d * (k - 1) - 1) // s + 1
...
requires = lambda *a, **k: (lambda cls: cls)
...
auto_docstring = lambda **k: (lambda c: c)
from transformers.models.auto import CONFIG_MAPPING, AutoConfig
...
class HiggsAudioV2TokenizerConfig(PreTrainedConfig):
    ...
    def from_dict(cls, config_dict, **kwargs):
        ...
        if "acoustic_model_config" in config_dict and isinstance(config_dict["acoustic_model_config"], dict):
            mtype = config_dict["acoustic_model_config"].get("model_type", "dac")
            config_dict["acoustic_model_config"] = CONFIG_MAPPING[mtype](**config_dict["acoustic_model_config"])
        if "semantic_model_config" in config_dict and isinstance(config_dict["semantic_model_config"], dict):
            mtype = config_dict["semantic_model_config"].get("model_type", "hubert")
            config_dict["semantic_model_config"] = CONFIG_MAPPING[mtype](**config_dict["semantic_model_config"])
...
HiggsAudioV2TokenizerConfig.to_diff_dict = lambda self: self.to_dict()


import torch.nn.init as init
from transformers import PreTrainedModel as PreTrainedAudioTokenizerBase, AutoModel
from transformers.utils import ModelOutput
